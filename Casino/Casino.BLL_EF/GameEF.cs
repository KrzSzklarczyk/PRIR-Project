...
namespace Casino.BLL_EF
{
    public class GameEF : IGame
    {
        ...
        public async Task<GameResponseDTO> GetGameInfo(int gameId)
        {
            var xd = await context.Games.FirstOrDefaultAsync(x => x.GameId == gameId);
            if (xd == null) { return null; }
            return mapper.Map<GameResponseDTO>(xd);
        }

        public async Task<BanditResponseDTO> GetBanditInfo(int gameId)
        {
            var xd = await context.Bandits.FirstOrDefaultAsync(x => x.BanditId == gameId);
            if (xd == null) { return null; }
            return mapper.Map<BanditResponseDTO>(xd);
        }

        public async Task<RouletteResponseDTO> GetRouletteInfo(int gameId)
        {
            var xd = await context.Roulettes.FirstOrDefaultAsync(x => x.RouletteId == gameId);
            if (xd == null) { return null; }
        ...
        public async Task<bool> PlayRoulette(UserTokenResponse token, RouletteRequestDTO roulette)
        {
            ...
            {
                Description = "test",
                betnumber = roulette.betNumber,
                Black = roulette.black,
                number = roulette.roll,
                Red = roulette.red
            };
        ...
        public async Task<bool> PlayBandit(UserTokenResponse token, BanditRequestDTO bandit)
        {
            ...
            Game game = new Game
            {
                ...
                MinBet = 25,
                StartDate = DateTime.UtcNow,
                amount = bandit.betAmount
            };

            Bandit bandit1 = new Bandit
            {
                Description = "test",
                Position1 = bandit.pos1,
                Position2 = bandit.pos2,
                Position3 = bandit.pos3
            };
