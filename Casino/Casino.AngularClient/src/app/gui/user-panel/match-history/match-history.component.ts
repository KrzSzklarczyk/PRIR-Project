...
export class MatchHistoryComponent implements OnInit {
  ...
  getBanditData(id: number): Observable<BanditResponseDTO> {
    ...
    if (this.cred.accessToken === '' || this.cred.refreshToken === '') {
      ...
      const url = `https://${addr.ipaddr}:7063/Game/Bandit/${id}`;
  ...
  getGameData(id: number): Observable<GameResponseDTO> {
    ...
    if (this.cred.accessToken === '' || this.cred.refreshToken === '') {
      ...
      const url = `https://${addr.ipaddr}:7063/Game/Game/${id}`;
  ...
  getRouletteData(id: number): Observable<RouletteResponseDTO> {
    ...
    if (this.cred.accessToken === '' || this.cred.refreshToken === '') {
      ...
      const url = `https://${addr.ipaddr}:7063/Game/Roulette/${id}`;
  ...
  getResultData(): void {
    ...
    this.http.post<ResultResponseDTO[]>(`https://${addr.ipaddr}:7063/Result/GetUserResult`, this.cred, {
      headers: new HttpHeaders({ 'Content-Type': 'application/json' }),
    }).subscribe(results => {
