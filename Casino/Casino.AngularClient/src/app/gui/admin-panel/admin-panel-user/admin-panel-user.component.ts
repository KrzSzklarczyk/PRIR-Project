...
export class AdminPanelUserComponent implements OnInit {
  ...
  GetHistory(id: number): void {
    ...
    const url = `${addr.ipaddr}/Transaction/History/${id}`;
  ...
  RemoveUser(id: number): void {
    ...
    const url = `${addr.ipaddr}/Account/RemoveUser/${id}`;
  ...
  GetAllUsers(): void {
    ...
    this.http.post<UserResponseDTO[]>(`${addr.ipaddr}/Account/getAllUsers`, this.cred, {
      headers: new HttpHeaders({ "Content-Type": "application/json" })
    }).subscribe({
  ...
  getMatchHistory(userId: number): void {
    ...
    const url = `${addr.ipaddr}/Result/GetUserResult/${userId}`;
